    ...
    return {
        ...
        updateTaskUtil : async (req) => {
            ...
            task.commentInfo = req?.body?.commentInfo;
             ...
             try{
                let t = await taskInfoModel.findOne({"_id" : taskId });
                if(!t) throw new Error("Invalid task Id  : "+ taskId + " not exists.");
                t.projectId = task.projectId;
                t.assigneedById = task.assigneedById;
                t.assigneedBy = task.assigneedBy;
                t.stage = task.stage;
                t.assigneeId = task.assigneeId;
                t.assignee = task.assignee;
                t.dueDate = task.dueDate;
                t.description = task.description;
                t.likeInfo = task.likeInfo;
                t.commentInfo = task.commentInfo; 
                t.save();
             }catch(e){
                throw new Error("Invalid task Id  : "+ taskId + " not exists. + error : "+e);
             }
